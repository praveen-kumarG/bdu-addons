...
class Job(models.Model):
    ...
    @api.one
    @api.depends('edition_ids', 'booklet_ids', 'waste_total', 'net_quantity')
    def _compute_all(self):
        ...
        self.total_mass = sum(booklet.calculated_mass for booklet in self.booklet_ids) / 1000
        self.perc_waste = self.waste_total / self.net_quantity * 100 if self.net_quantity > 0 else False
        self.production_start = self.edition_ids and min(line.production_start for line in self.edition_ids) or False
        self.production_stop = self.edition_ids and max(line.production_stop for line in self.edition_ids) or False
    ...
    total_mass = fields.Integer('Total Mass', compute='_compute_all')
    perc_waste = fields.Integer('% Waste', compute='_compute_all')
    ...
    def action_create_job(self):
        ...
        for RegistryXML1, listXML3 in merge_xml3.iteritems():
            # no picking exists
            if not RegistryXML1.job_id.picking_id :
