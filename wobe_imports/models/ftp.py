...
class Registry(models.Model):
    ...
    file_track_date = fields.Datetime('File Track Date')
                ...
                if header.get('SenderId') == 'WOBEWebportal' and header.get('Type') == 'CreateJob':
                    ...
                    if "xml1a" in filename:
                        ...
                        vals.update({
                            ...
                            'file_track_date':xml1aDt['file_track_date'],
                        })
                        ...
                        vals.update({
                            ...
                            'file_track_date': xml1bDt['file_track_date'],
                        })
                vals.update({
                    'name': filename,
    ...
    def convert_track_date(self, trackDate):
        '''
            :param trackDate: eg, 2018-08-23T12:45:32
            :return: date time in UTC format
        '''
        trackDate = datetime.strptime(trackDate, "%Y-%m-%dT%H:%M:%S").strftime('%Y-%m-%d %H:%M:%S')
        return self.env['wobe.job'].convert_TZ_UTC(trackDate)
    ...
    def parse_xml1a(self, root):
        ...
        '''
        data = {}
        ifra = "{http://www.ifra.com/IfraTrack/imf/4.1}"
        head = ifra + "Head"
        time = ifra + "Time"
        ...
        track_date = root.find(head).find(time).text or False
        if track_date:
            data['file_track_date'] = self.convert_track_date(track_date)
